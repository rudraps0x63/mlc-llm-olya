...
from .whisper_quantization import awq_quant
...
def huggingface(model_config: WhisperConfig, quantization: Quantization) -> ExternMapping:
    ...
    mlc_name = ["proj_out.weight"]
    src_name = ["model.decoder.embed_tokens.weight"]
...
def awq(model_config: WhisperConfig, quantization: Quantization) -> ExternMapping:
    """Returns a parameter mapping that maps from the names of MLC LLM parameters to
